...
describe("extractItems bullet cleaning", () => {
  const bulletPreview = (bulletLatex: string) =>
    extractItems(`\\resumeItem{${bulletLatex}}`)[0];

  it("strips \\underline{} instead of leaking it", () => {
    expect(
      bulletPreview(
        "Built \\underline{interactive product tours} and led the redesign",
      ),
    ).toBe("Built interactive product tours and led the redesign");
  });
  ...
  it("handles nested formatting macros", () => {
    expect(bulletPreview("\\textbf{\\underline{Shipped}} a feature")).toBe(
      "Shipped a feature",
    );
  });
  ...
  it("strips other common text macros (textsc, texttt, textcolor, url)", () => {
    expect(
      bulletPreview(
        "Used \\texttt{grep} and \\textsc{Sql} via \\url{example.com}",
      ),
    ).toBe("Used grep and Sql via example.com");
    expect(bulletPreview("Marked \\textcolor{red}{urgent} items")).toBe(
      "Marked urgent items",
    );
  });
  ...
  it("resolves escaped special characters", () => {
    ...
    ).toBe("Cut costs 20% on a $50k budget for R&D #1 priority");
  });

  it("drops an unknown macro's name but keeps its braced content", () => {
    expect(
      bulletPreview("Led a \\somefuturemacro{cross-functional} team"),
    ).toBe("Led a cross-functional team");
  });

  it("handles line breaks with an optional spacing arg", () => {
    expect(bulletPreview("Line one\\\\[2pt]Line two")).toBe(
      "Line one Line two",
    );
  });
